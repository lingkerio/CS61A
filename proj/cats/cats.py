...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    ans_list = [item for item in paragraphs if select(item)]
    if k > len(ans_list) - 1:
        return ''
    return ans_list[k]
    # END PROBLEM 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    diff_list = [diff_function(user_word, item, limit) for item in valid_words]
    similar_word, similar_diff = min(zip(valid_words, diff_list), key=lambda item : item[1])
...
def fastest_words(game):
    ...
    for i in word_indices:
        word_time = [times[player][i] for player in player_indices]
        index = min(player_indices, key=lambda player : word_time[player])
        fastest[index].append(words[i])
    return fastest
